...
def get_rech_mon_df(self):
    ...
    for selectedDate in dateList_f:
        ...
        self.dlg.raise_()
    big_df.insert(0, 'grid_id', value=range(1, len(big_df) + 1))
    big_df.to_csv(os.path.join(exported_folder, "mf_recharge_mon_table.csv"), index=False)
    return big_df


def get_mf_rech_day_df(self):
    self.main_messageBox("Reading ...", "We are going to read the 'swatmf_out_MF_recharge file ...")
    QSWATMOD_path_dict = self.dirs_and_paths()
    stdate, eddate, stdate_warmup, eddate_warmup = self.define_sim_period()
    wd = QSWATMOD_path_dict['SMfolder']
    startDate = stdate.strftime("%m-%d-%Y")
...
def get_mf_rech_year_df(self):
    self.main_messageBox("Reading ...", "We are going to read the 'swatmf_out_MF_recharge_yearly file ...")
    QSWATMOD_path_dict = self.dirs_and_paths()
    stdate, eddate, stdate_warmup, eddate_warmup = self.define_sim_period()
    wd = QSWATMOD_path_dict['SMfolder']
    startDate = stdate.strftime("%m-%d-%Y")
